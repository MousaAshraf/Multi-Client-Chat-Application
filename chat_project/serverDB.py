...
class ChatServer:
    ...
    def remove_client(self, username: str, notify: bool = True):
        ...
        with self.clients_lock:
            if username in self.clients:
                ...
                # Notify other users
                if notify:
                    self.broadcast(f"[SERVER] {username} left the chat", sender=username, store_message=False)
    ...
    def send_message_history(self, username: str, client_socket: socket.socket):
        """
        Send message history to reconnecting user
        Implements: Message Persistence requirement
        """
        try:
            # Small delay to ensure receive thread is ready
            time.sleep(0.1)
            
            # Get user's message history
            messages = self.db.get_user_messages(username, limit=100)
            
            if messages:
                # Build complete history message as one string
                history_parts = [f"\n--- Message History ({len(messages)} messages) ---\n"]
                
                # Add messages in chronological order
                for sender, receiver, message, timestamp, msg_type in messages:
                    ...
                    if msg_type == 'private':
                        if receiver == username:
                            ...
                            formatted = f"[{timestamp_str}] [PRIVATE] {sender}: {message}\n"
                        else:
                            # Message sent by this user
                            formatted = f"[{timestamp_str}] [PRIVATE] You -> {receiver}: {message}\n"
                    else:
                        # Group message
                        formatted = f"[{timestamp_str}] {sender}: {message}\n"
                    
                    history_parts.append(formatted)
                
                # Add footer
                history_parts.append("--- End of History ---\n")
            ...
            logging.error(f"Error sending message history to {username}: {e}")
    
    # ==================== MESSAGE ROUTING ====================
    
    def broadcast(self, message: str, sender: Optional[str] = None, store_message: bool = True):
        """
        Group Messaging: Broadcast to all connected clients
        Implements: Group Messaging requirement
        Args:
            message: Formatted message to broadcast
            sender: Username of sender (None for server messages)
            store_message: Whether to store message in database (default: True)
        """
        # Ensure message ends with newline for proper client display
    ...
    def private_message(self, target_user: str, message: str, sender: str):
        ...
        with self.clients_lock:
            ...
            if target_user in self.clients:
                try:
                    # Store message in database (private message)
                    self.db.save_message(sender, message, receiver=target_user)
                ...
                # User not found - still store the message for when they reconnect
                self.db.save_message(sender, message, receiver=target_user)
    ...
    def handle_client(self, client_socket: socket.socket, addr):
        ...
        try:
            ...
            # Step 3.5: Send message history
            self.send_message_history(username, client_socket)
            
            # Step 4: Notify others
            self.broadcast(f"[SERVER] {username} joined the chat", sender=username, store_message=False)
    ...
    def route_message(self, username: str, message: str):
        ...
        # Group message
            ...
            self.db.save_message(username, message, receiver=None)
            
            # Broadcast formatted message
            group_msg = f"{username}: {message}"
            self.broadcast(group_msg, sender=username, store_message=False)
